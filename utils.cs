...
namespace wowCVAHK
{
    ...
    internal class FullScreenOverlay : Form
    {
        ...
        private Bitmap CaptureScreen()
        {
            ...
            // 获取 DPI 缩放比例
            float scaleFactor = Utils.GetScalingFactor();

            // 根据缩放比例调整截图区域
            int scaledWidth = (int)(screenBounds.Width * scaleFactor);
            int scaledHeight = (int)(screenBounds.Height * scaleFactor);

            // 创建一个适应缩放的位图
            Bitmap screenshot = new Bitmap(scaledWidth, scaledHeight);

            // 使用 Graphics 对象从屏幕中拷贝图像到位图
            ...
            //{
            //    g.CopyFromScreen(0, 0, 0, 0, bmpScreenCapture.Size); // 截取整个屏幕
            //}
